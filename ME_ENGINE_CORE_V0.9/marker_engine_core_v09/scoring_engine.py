...
from marker_models import MarkerMatch, MarkerCategory, MarkerSeverity
from chunk_models import TextChunk
...
class ScoringEngine:
    ...
    def _initialize_default_models(self):
        ...
        self.models["manipulation_index"] = ScoringModel(
            ...
            category_weights={
                MarkerCategory.MANIPULATION.value: 2.0,
                MarkerCategory.GASLIGHTING.value: 3.0,
                MarkerCategory.EMOTIONAL_ABUSE.value: 2.5,
                MarkerCategory.LOVE_BOMBING.value: 1.5,
                MarkerCategory.FRAUD.value: 3.0,
                MarkerCategory.POSITIVE.value: -1.0,  # Positive Marker reduzieren Score
                MarkerCategory.EMPATHY.value: -0.5,
                MarkerCategory.SUPPORT.value: -0.5
            },
        ...
        self.models["relationship_health"] = ScoringModel(
            ...
            category_weights={
                MarkerCategory.POSITIVE.value: 2.0,
                MarkerCategory.EMPATHY.value: 2.5,
                MarkerCategory.SUPPORT.value: 2.0,
                MarkerCategory.CONFLICT_RESOLUTION.value: 1.5,
                MarkerCategory.MANIPULATION.value: -2.0,
                MarkerCategory.GASLIGHTING.value: -3.0,
                MarkerCategory.EMOTIONAL_ABUSE.value: -2.5,
                MarkerCategory.FRAUD.value: -3.0
        ...
        self.models["fraud_probability"] = ScoringModel(
            ...
            category_weights={
                MarkerCategory.FRAUD.value: 3.0,
                MarkerCategory.FINANCIAL_ABUSE.value: 2.5,
                MarkerCategory.LOVE_BOMBING.value: 1.5,
                MarkerCategory.MANIPULATION.value: 1.0,
                MarkerCategory.POSITIVE.value: -0.5,
                MarkerCategory.EMPATHY.value: -0.3
            },
            thresholds={
                "critical": 7.0,
                "warning": 5.0,
        ...
        self.models["communication_quality"] = ScoringModel(
            ...
            category_weights={
                MarkerCategory.POSITIVE.value: 1.5,
                MarkerCategory.EMPATHY.value: 2.0,
                MarkerCategory.SUPPORT.value: 1.5,
                MarkerCategory.CONFLICT_RESOLUTION.value: 2.0,
                MarkerCategory.BOUNDARY_SETTING.value: 1.0,
                MarkerCategory.SELF_CARE.value: 0.5,
                MarkerCategory.MANIPULATION.value: -1.5,
                MarkerCategory.GASLIGHTING.value: -2.0,
                MarkerCategory.EMOTIONAL_ABUSE.value: -2.0
            },
            inverse_scale=True
        )
    ...
    def _calculate_chunk_score(
        ...
        for match in matches:
            if match.category.value in model.category_weights:
                weight = model.category_weights[match.category.value]
                severity_mult = model.severity_multipliers.get(
                    match.severity.value,
                    1.0
                ) if model.severity_multipliers else 1.0
    ...
    def compare_scores(
        ...
        """Vergleicht zwei Scoring-Ergebnisse."""
        comparisons = []
        
        active_models = self._get_active_models(model_ids)
        
        for model in active_models:
            score_type = model.type.value
            
            agg1 = result1.aggregated_scores.get(score_type)
            agg2 = result2.aggregated_scores.get(score_type)
            
            if agg1 and agg2:
                delta = agg2.average_score - agg1.average_score
                
                if delta > 0.5:
                    change = "Verbesserung"
                elif delta < -0.5:
                    change = "Verschlechterung"
                else:
                    change = "Stabil"
                ...
                comparisons.append(ScoreComparison(
                    model_id=model.id,
                    score_type=model.type,
                    score1=agg1.average_score,
                    score2=agg2.average_score,
                    delta=delta,
                    change=change
                ))
